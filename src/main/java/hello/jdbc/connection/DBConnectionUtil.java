...
public class DBConnectionUtil {
    public static Connection getConnection() {
        try {
             ...
             * 필요한 값만(URL, USERNAME, PASSWORD)만 넣어준다면 Connection 인터페이스의 구현체들(드라이버)로부터 DB 연결을 할 수 있다.
             * (각 DB 사들이 자바 인터페이스에 맞게 커네션 반환 메서드 구현했다.)
             * 이로부터 개발자가 각각의 데이터베이스마다 커넥션 연결, SQL 전달, 그리고 그 결과를 응답 받는 방법을 새로 학습할 필요가 없어졌다.

             * 데이터베이스에 연결하려면 JDBC(import javax)가 제공하는 DriverManager.getConnection(..) 를 사용하면 된다. (데이터베이스 커넥션 반환 및획득)
             * 어떻게 연결이 되냐
             * build.gradle 에 h2 드라이버를 라이브러리에 등록하고
             * getConnection()이 알아서 라이브러리에 등록된 드라이버 목록들에 일단 다 커넥션 요청을 한다. (getConnection이 알아서 인지한다.)
             * 각각의 드라이버는 처리가 가능한지 확인을 한다 -> 가령 jdbc:h2로 시작하면 h2 드라이버가 처리할 것이다.
             */
